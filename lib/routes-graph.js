...
module.exports = class RoutesGraph extends Graph {
   ...
   * @returns {number}            - Number of routes that match criteria.
   */
  _exactStops(from, to, stops, showRoutes) {
    let routes, numStops, cur, edges, numEdges, len, next, parentMap;
    const edgesAtStop = [1];
    const q = [];

    if (showRoutes) parentMap = new Map();
  ...
  /**
   * _maxDistance - Finds the number of routes from 'from' to 'to' that are
   *                < 'maxDistance' away.
   *
   * @param  {Edge} from
   * @param  {string} to          - String of length 1 e.g 'A'.
   * @param  {type} maxDistance   - Distance limit.
   * @param  {boolean} showRoutes - if true prints routes.
   * @returns {number}            - The number of routes that match criteria.
   */
  _maxDistance(from, to, maxDistance, showRoutes) {
    const edgesAtStop = [1];
    const q = [];
    let parentMap, routes, numStops, numEdges, len, next, edges;
    let done = false;
  ...
  /**
   * _maxDistanceRecursive - Finds the number of routes from 'from' to 'to' that
   *                         are < 'maxDistance' away.
   *
   * @param  {Edge} cur
   * @param  {string} to            - String of length 1 e.g 'A'.
   ...
   * @param  {string} from       - String of length 1 e.g 'A'.
   * @param  {string} to         - String of length 1 e.g 'A'.
   * @param  {boolean} showRoute - if true prints route.
