...
module.exports = class RoutesGraph extends Graph {
  ...
  _maxStops (from, to, maxStops, showRoutes) {
    let routes, stops, cur, edges, numNodesAtStop, numNodesAtNextStop;
    const parentMap = new Map();
    const q = new Queue();

    routes = stops = numNodesAtStop = 0;
    ...
    while (q.size && stops < maxStops) {
      stops++;
      numNodesAtNextStop = 0;

      for (let i = 0; i < numNodesAtStop; i++) {
        cur = q.dequeue();
        if (cur.to === to) {
          routes++;
      ...
      numNodesAtStop = numNodesAtNextStop;
    }
